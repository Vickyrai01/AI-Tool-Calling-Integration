      ...
      <div className="absolute inset-x-0 top-0 bg-white rounded-b-xl shadow-xl border-b border-border p-3 max-h-[85vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-3">
          ...
          <button
            ...
            onClick={() => { onOpenConversations(); onClose(); }}
          >
            Conversaciones
          </button>
          ...
          <div className="border border-border rounded-md">
            ...
            <div className="px-2 pb-2 flex flex-col gap-1">
              <div className="px-2 py-1 text-[11px] text-muted uppercase">Dificultad</div>
              {difficulty.map((d) => (
                <button key={d.label} className="text-left px-2 py-2 rounded hover:bg-blue-50 text-sm" onClick={() => runPrompt(d.text)}>
                  {d.label}
                </button>
              ))}
              <div className="px-2 py-1 text-[11px] text-muted uppercase mt-2">Plantillas</div>
              {templates.map((t) => (
                <button key={t.label} className="text-left px-2 py-2 rounded hover:bg-blue-50 text-sm" onClick={() => runPrompt(t.text)}>
                  {t.label}
                </button>
              ))}
            </div>
          ...
          <button
            className={['w-full px-3 py-3 rounded-md border text-sm',
              debugOn ? 'border-primary text-primary bg-blue-50' : 'border-border bg-white',
            ].join(' ')}
      ...
      <div className="px-3 lg:px-4 py-2">
        <div className="flex items-center justify-between">
          ...
          <div className="text-center">
            <div className="text-sm lg:text-base leading-tight font-semibold">Bot de ejercicios</div>
            <div className="text-xs text-muted -mt-0.5">Ecuaciones lineales</div>
          </div>
