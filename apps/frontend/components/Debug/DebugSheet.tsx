      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-border rounded-t-lg shadow-xl p-3 max-h-[70vh] overflow-y-auto">
        ...
        {!meta && (
          <div className="text-xs text-muted">
            Debug activo. Enviá un mensaje para ver tools, tiempos y tokens
            aquí.
          </div>
        )}
          ...
          <div className="text-sm grid grid-cols-1 gap-3">
            ...
            <div>
              ...
              <span className="text-muted">
                initial {t.initialMs ?? "—"}ms · followup {t.followupMs ?? "—"}
                ms
              </span>
            </div>
            ...
            <div>
              ...
              {showInit ? (
                <pre className="text-xs bg-bg border border-border rounded p-2 whitespace-pre-wrap break-words max-h-40 overflow-auto">
                  {tokens.initial
                    ? JSON.stringify(tokens.initial, null, 2)
                    : "—"}
                </pre>
              ) : (
            ...
            <div>
              ...
              {showFollow ? (
                <pre className="text-xs bg-bg border border-border rounded p-2 whitespace-pre-wrap break-words max-h-40 overflow-auto">
                  {tokens.followup
                    ? JSON.stringify(tokens.followup, null, 2)
                    : "—"}
                </pre>
              ) : (
                <div className="text-xs text-muted">oculto</div>
            ...
            <div>
              ...
              <div className="flex flex-col gap-2">
                {tools.length === 0 && (
                  <div className="text-muted text-sm">sin tools</div>
                )}
                {tools.map((ev, i) => (
                  <div
                    key={i}
